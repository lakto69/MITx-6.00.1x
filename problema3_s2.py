...
rates. (Note: when your code is running on our servers, there are limits on the amount
of computing time each submission is allowed, so your observations from running
this experiment on the grading system might be limited to an error message complaining
about too much time taken.)

Well then, how can we calculate a more accurate fixed monthly payment than we did
in Problem 2 without running into the problem of slow code? We can make this program
...
for this payment value? $0 is the obvious answer, but you can do better than that.
...
than what we would've paid in monthly installments, because the interest was compounded
...
[http://en.wikipedia.org/wiki/Bisection_method]) to find the
smallest monthly payment to the cent (no more multiples of $10) such that
we can pay off the debt within a year. Try it out with large inputs, and notice
...
def pagando_bi(saldo, juros_anuais, valor_minimo, tempo=12):
    ...
    if tempo == 1:
        ...
        tempo -= 1
        return pagando_bi(novo_saldo, juros_anuais, valor_minimo, tempo)


def PayingDebtOff_bi(balance, annualInterestRate, tempo=12):
    taxa_mensal = annualInterestRate / 12
    valor_minimo = balance / 12  # A
    valor_maximo = (balance * (1 + taxa_mensal) ** 12) / 12  # B

    iteracao = 1
    while iteracao < 200:
        valor_a_verificar = (valor_minimo + valor_maximo) / 2  # C
        fc = pagando_bi(balance, annualInterestRate, valor_a_verificar)
        if abs(fc) <= 0.01:
            return round(valor_a_verificar, 2)
            break
        if sinal(fc) == sinal(pagando_bi(balance, annualInterestRate, valor_minimo)):
            valor_minimo = valor_a_verificar
        else:
            valor_maximo = valor_a_verificar
        iteracao += 1


print("Lowest Payment:", PayingDebtOff_bi(999999, 0.18))
# print('1º minimo', pagando_bi(320000, 0.2, 26666.66))
# print('1º máximo', pagando_bi(320000, 0.2, 32517.09))
# print('correto', pagando_bi(320000, 0.2, 29157.09))
